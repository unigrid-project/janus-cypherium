...
CzUNIGRIDWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "unigrid.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "unigridd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the UNIGRID and zUNIGRID money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in UNIGRID/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all UNIGRID specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-unigridstake=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-unigridstake=<n>", strprintf(_("Enable or disable staking functionality for UNIGRID inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zunigridstake=<n>", strprintf(_("Enable or disable staking functionality for zUNIGRID inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zUNIGRID minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zUNIGRID backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The UNIGRID organization"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDisableWallet) {
#endif
        if (SoftSetBoolArg("-unigridstake", false))
            LogPrintf("AppInit2 : parameter interaction: wallet functionality not enabled -> setting -unigridstake=0\n");
#ifdef ENABLE_WALLET
    }
#endif
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. UNIGRID Core is shutting down."));
...
#endif
    // Make sure only a single UNIGRID process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. UNIGRID Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // UNIGRID: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of UNIGRID Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart UNIGRID Core to complete") << "\n";
        ...
        zwalletMain = new CzUNIGRIDWallet(pwalletMain->strWalletFile);
...
//    if (nAnonymizeUnigridAmount < 2) nAnonymizeUnigridAmount = 2;
    ...
    LogPrintf("Anonymize UNIGRID Amount %d\n", nAnonymizeUnigridAmount);
    ...
    /* Denominations
       ...
       For example:
       1UNIGRID+1000 == (.1UNIGRID+100)*10
       10UNIGRID+10000 == (1UNIGRID+1000)*10
    */
