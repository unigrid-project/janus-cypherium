                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zUNIGRID ought to be enough for anybody." - Bill Gates, 2017
    ui->zUNIGRIDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzUNIGRIDSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zUNIGRIDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzUNIGRID_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUNIGRID is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zUNIGRID, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zUNIGRID...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zUNIGRID in ") +
...
void PrivacyDialog::on_pushButtonSpendzUNIGRID_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUNIGRID is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zUNIGRID, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zUNIGRID
        sendzUNIGRID();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zUNIGRID
    sendzUNIGRID();
}

void PrivacyDialog::on_pushButtonZUnigridControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZUnigridControlDialog* zUnigridControl = new ZUnigridControlDialog(this);
    zUnigridControl->setModel(walletModel);
    zUnigridControl->exec();
}

void PrivacyDialog::setZUnigridControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzUnigridSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzUNIGRID()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Unigrid Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zUNIGRIDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zUNIGRID is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " UNIGRID </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zUNIGRID</b>";
    ...
    // use mints from zUNIGRID selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZUnigridControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZUnigridControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zUNIGRID require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zUNIGRID"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZUNIGRID_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zUNIGRID require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zUNIGRID"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zUNIGRID was spent successfully update the addressbook with the label
    ...
    ZUnigridControlDialog::setSelectedMints.clear();
    ui->labelzUnigridSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zUNIGRID Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Unigrid, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zUNIGRID Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zUNIGRIDpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zUNIGRID </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zUNIGRID "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zUNIGRID "));
    ui->labelzUNIGRIDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zUNIGRID </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zUNIGRID </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzUNIGRIDSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzUNIGRID->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zUNIGRID
        ui->pushButtonMintzUNIGRID->setEnabled(false);
        ui->pushButtonMintzUNIGRID->setToolTip(tr("zUNIGRID is currently disabled due to maintenance."));

        // Spend zUNIGRID
        ui->pushButtonSpendzUNIGRID->setEnabled(false);
        ui->pushButtonSpendzUNIGRID->setToolTip(tr("zUNIGRID is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zUNIGRID
        ui->pushButtonMintzUNIGRID->setEnabled(true);
        ui->pushButtonMintzUNIGRID->setToolTip(tr("PrivacyDialog", "Enter an amount of UNIGRID to convert to zUNIGRID", 0));

        // Spend zUNIGRID
        ui->pushButtonSpendzUNIGRID->setEnabled(true);
        ui->pushButtonSpendzUNIGRID->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
