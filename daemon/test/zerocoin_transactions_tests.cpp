...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpivTracker = unique_ptr<CzUNIGRIDTracker>(new CzUNIGRIDTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZUNIGRID_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZUNIGRID_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
