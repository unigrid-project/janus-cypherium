...
const ImportAccount = (props) => {
    const [selections, setSelections] = useState([
        { name: "mnemonic", selected: true, key: 0 },
        { name: "private key", selected: false, key: 1 },
        { name: "keystore", selected: false, key: 2 }
    ]);
    const walletService = new WalletService();
    const enterMnemonicCopy = "Please enter your mnemonics in original order, seperated by spaces.";
    ...
    function importWallet() {
        ...
        if (checkMnemonic() === true) {
            ...
            if (walletName === "") {
                ...
                let account = walletService.fromMnemonic(mnemonic).then((result) => {
    ...
    function checkMnemonic() {
        ...
        if (mnemonicList.length !== 12) {
            setWarningMessage("Mnemonic length is too short");
        ...
        if (!walletService.validateMnemonic(mnemonic)) {
            setWarningMessage("Invalid mnemonic");
            return false;
        }
        return true;
    }
