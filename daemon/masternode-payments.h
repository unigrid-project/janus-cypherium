...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZUNIGRIDStake);
...
public:
    ...
    bool IsTransactionValid(int blockVersion, const CTransaction& txNew);
...
public:
    ...
    bool IsTransactionValid(int blockVersion, const CTransaction& txNew, int nBlockHeight);
    ...
    void ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);
    std::string GetRequiredPaymentsString(int nBlockHeight);
    void FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZUNIGRIDStake);
